...
class Timer:
    ...
    def stopAndLog(self, abort=False):
        ...
        self.stop()
        if (abort):
            logging.warning("Calculation aborted after {}".format(Timer.__millisToStr(self.difference)))
        return str(self)

    def __str__(self):
        if (self.startTime is not None):
            if (self.difference is None):
                return "Timer started {} ago".format(Timer.__millisToStr(time.perf_counter() - self.startTime))
            else:
                return Timer.__millisToStr(self.difference)
