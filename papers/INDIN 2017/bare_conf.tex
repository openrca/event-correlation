...
By using normalization, a closed measure \(\text{Cor}^*(A, B) \in [0, 1]\) for the similarity of two sample sets is obtained.
...
Figure \ref{fig:scen1} shows the estimated distributions for both matchers. Even though nearly all event pairs overlap, \ac{ICE} is able to estimate the true distribution with a very low energy distance of $0.0509$. Even with several retries, \textit{lagEM} was not able to estimate the correct distribution, which is also reflected in the calculated distance of $0.3209$. Zeng et al. reported the same behavior but were able to solve it by using longer sequences \cite{Zeng2015}.
